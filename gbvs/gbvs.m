...
for i=1:length(mapnames)
    lmaps2 = {};
    for j=1:length(param.levels), lmaps2{j}=0; end
    cmaps2{i}=lmaps2;
end
Nfmap2 = cmaps2;
...
for j=1:length(norm_maps)
  map = norm_maps{j}.map;
  fmapi = norm_maps{j}.maptype(1);
  lev = norm_maps{j}.maptype(3);
  levi = find(param.levels==lev);
  Nfmap2{fmapi}{levi} = Nfmap2{fmapi}{levi} + 1;
  cmaps2{fmapi}{levi} = cmaps2{fmapi}{levi} + map;
end
%%% divide each feature channel by number of maps in that channel
for fmapi = 1 : length(mapnames)
   for levi=1:length(param.levels)
    if ( param.normalizeTopChannelMaps) 
      mymessage(param,'Performing additional top-level feature map normalization.\n');
      if ( param.normalizationType == 1 )
          algtype = 4;
          [cmaps2{fmapi}{levi},tmp] = graphsalapply( cmaps2{fmapi}{levi} , grframe, param.sigma_frac_norm, param.num_norm_iters, algtype , param.tol );
      elseif ( param.normalizationType == 2 )
          algtype = 1;
          [cmaps2{fmapi}{levi},tmp] = graphsalapply( cmaps2{fmapi}{levi} , grframe, param.sigma_frac_norm, param.num_norm_iters, algtype , param.tol );
      else
        cmaps2{fmapi}{levi} = maxNormalizeStdGBVS( cmaps2{fmapi}{levi} );
      end
    end
    comb_norm_maps2{fmapi}{levi} = cmaps2{fmapi}{levi};
   end
end
...
scale_maps = {};
for i = 1 : length(mapnames)
  for j=1:length(param.levels)
    scale_maps{i}{j} = mat2gray(comb_norm_maps2{i}{j});
  end
end
