function NSS_tuned1 = testSaliencymap(typei, ALLFeatures, faceFeatures, kyokai, weight, testingdata, fromfirst)
...
for testidx=1:length(testingdata)
    ...
    for i=1:size(testingdata{testidx}.sacinfo)
        ...
        for k=1:length(kyokai)
            ...
            elseif(typei == 3)
                num_feat = 4;
                result_tuned1(calIdx) = ...
                weight(num_feat*(k-1)+1).*color(calIdx) + weight(num_feat*(k-1)+2).*intensity(calIdx) + weight(num_feat*(k-1)+3).*orientation(calIdx) + weight(num_feat*(k-1)+4).*face(calIdx);
            elseif(typei == 4)
                num_feat = 10;
                result_tuned1(calIdx) = ...
                weight(1).*c1(calIdx) + weight(2).*c3(calIdx) + weight(3).*c3(calIdx) + ...
                weight(4).*i1(calIdx) + weight(5).*i2(calIdx) + weight(6).*i3(calIdx) + ...
                weight(7).*o1(calIdx) + weight(8).*o2(calIdx) + weight(9).*o3(calIdx) + weight(10).*face(calIdx);
            elseif(typei == 5)
                num_feat = 4;
                result_tuned1(calIdx) = ...
                weight(1).*color(calIdx) + weight(2).*intensity(calIdx) + weight(3).*orientation(calIdx) + weight(4).*face(calIdx);
            end
        end
    ...
    clear c1 c2 c3 i1 i2 i3 o1 o2 o3 color intensity orientation face
