...
function [data,Fix,Sac] = getFixations(data,allowedNaNs)
...
DegPerSec = 22;                      % Acceleration threshold
pxPerDeg =  get_distance(1);                      % based on current ET setup (display= 1024x768px = 40.5x30cm; distance=75cm)
accelCrit = pxPerDeg * DegPerSec;   %(34 px/deg)*(6 deg/sec) velocity diff. btwn t=0 & t=(stepSize/samplingRate)
winSize = 3;                        %4 samples = 16.667 ms
stepSize = 1;                       %1 samples =  4.167 ms
samplingRate = 60;

%% Minimum duration of fixation:
FixDurCut = 100;                   %Shortest fixation allowed [ms]
