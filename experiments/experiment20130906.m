...
% load('../Resource/sampleInfo/sampleStat1st_3region.mat');
% load('../Resource/sampleInfo/sampleInfoSaccade1_3Region.mat');
...
for subjecti = 1:1
    ...
    opt = opt_base;
    opt.n_region = 3; %fixed, do not change it.
    opt.enable_angle = 0;
    fprintf('========================================================= angle: %d region: %d\n', opt.enable_angle, opt.n_region);
    RET = {};
    opt.thresholdLength = Info.thresholdLength{opt.n_order_fromfirst};
    [RET.mInfo_tune, RET.mNSS_tune, RET.opt_ret] = calcMainPerSubject20130906(opt, EXPALLFixations, ALLFeatures, faceFeatures, sampleinfo,sampleinfoStat, subjecti);
...
savefile = sprintf('../Output/storage/EXP20130906_%s.mat', info.time_stamp);
